...
using Azure.Mcp.Tools.WebPubSub.Commands;
...
namespace Azure.Mcp.Tools.WebPubSub.UnitTests.Runtime;

public class RuntimeGetCommandTest
{
    ...
    public RuntimeGetCommandTest()
    {
        _webPubSubService = Substitute.For<IWebPubSubService>();
        _logger = Substitute.For<ILogger<RuntimeGetCommand>>();

        var collection = new ServiceCollection().AddSingleton(_webPubSubService);
    ...
    [InlineData("--subscription sub1 --resource-group rg1", true)] // webpubsub is optional
    [InlineData("--subscription sub1 --resource-group rg1 --webpubsub w1", true)] // all provided
    [InlineData("--resource-group rg1 --webpubsub w1", false)] // subscription is required
    [InlineData("", false)]
    public async Task ExecuteAsync_ValidatesInputCorrectly(string args, bool shouldSucceed)
    {
        ...
        if (shouldSucceed)
        {
            ...
            _webPubSubService.GetRuntimeAsync(
                Arg.Any<string>(),
                Arg.Any<string?>(),
                Arg.Any<string?>(),
                Arg.Any<string?>(),
    ...
    public async Task ExecuteAsync_ReturnsEmptyWhenNoRuntimes()
    {
        ...
        var result = JsonSerializer.Deserialize(json, WebPubSubJsonContext.Default.RuntimeGetCommandResult);
